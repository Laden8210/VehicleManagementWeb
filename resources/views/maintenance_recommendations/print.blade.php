        ...
        <div class="mb-4">
            <div class="flex justify-between mb-2">
                ...
                <div class="flex items-center">
                    ...
                    <b
                        class="text-sm">{{ \Carbon\Carbon::parse($maintenanceRecommendations->RequestDate)->format('F j, Y') }}</b>
                </div>
            </div>
            <div class="flex items-center">
                <label class="block font-semibold text-sm mr-2">Type of Vehicle / Equipment:</label>
                <b class="text-sm">{{ $maintenanceRecommendations->vehicle->Make }}
                    {{ $maintenanceRecommendations->vehicle->Series }}</b>
            </div>
            <div class="flex items-center">
                <label class="block font-semibold text-sm mr-2">Plate Number / Serial Number:</label>
                <b class="text-sm">{{ $maintenanceRecommendations->vehicle->MvfileNo }}</b>
            </div>
        </div>
        ...
        <div class="mb-4">
            <table class="w-full">
                ...
                <tbody>
                    ...
                    @if(!empty($maintenanceRecommendations->Issues))
                                        @php
                                            $decodedIssues = json_decode($maintenanceRecommendations->Issues, true);
